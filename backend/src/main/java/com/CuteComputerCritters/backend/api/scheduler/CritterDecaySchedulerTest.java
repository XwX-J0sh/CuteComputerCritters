...
@Service
public class CritterDecaySchedulerTest {

    private CritterRepository critterRepository;

    public CritterDecaySchedulerTest(CritterRepository critterRepository) {
        this.critterRepository = critterRepository;
    }

    @Scheduled(fixedRate = 7000) // every minute is 60000
    public void updateEvolutionStagesAndDecayStats() {
        ...
        for (Critter critter : activeCritters) {
            ...
            //After first 2min pet gets sick once to introduce the sickness game mechanic
            if(evolutionStage == 1 && critter.getTotalActiveTime() >= 120) {
                critter.setHealthy(false);
            }

            //after 10 min hit evolution stage 2
            if ((evolutionStage == 1) && critter.getTotalActiveTime() >= 600) {
            ...
            if (critter.getCalledSince() != null) {
                ...
                timeSinceLastCall = Duration.ofMinutes(3);
            }
